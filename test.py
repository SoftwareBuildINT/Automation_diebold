...
def job():
    ...
    schedule.every().day.at("15:20").do(read_and_send_messages, 'ALL_SITES(8_TO_8).csv', turn_off=False)
    schedule.every().day.at("20:00").do(read_and_send_messages, 'ALL_SITES(8_TO_8).csv', turn_off=True)
    schedule.every().day.at("15:20").do(read_and_send_messages, 'Hyderbad(8_TO_9,30).csv', turn_off=False)
